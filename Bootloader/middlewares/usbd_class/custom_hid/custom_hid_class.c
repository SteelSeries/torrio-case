...
static void usb_hid_buf_process(void *udev, uint8_t *report, size_t len);
...
static usb_sts_type class_init_handler(void *udev)
{
  ...
  /* open custom hid in endpoint */
  usbd_ept_open(pudev, USBD_CUSTOM_HID_IN_EPT2, EPT_INT_TYPE, USBD_CUSTOM_IN_MAXPACKET_SIZE);

  /* open custom hid in endpoint */
  usbd_ept_open(pudev, USBD_CUSTOM_HID_IN_EPT3, EPT_INT_TYPE, USBD_CUSTOM_IN_MAXPACKET_SIZE);
...
static usb_sts_type class_clear_handler(void *udev)
{
  ...
  /* close custom hid in endpoint */
  usbd_ept_close(pudev, USBD_CUSTOM_HID_IN_EPT2);

  /* close custom hid in endpoint */
  usbd_ept_close(pudev, USBD_CUSTOM_HID_IN_EPT3);
      ...
      switch(setup->bRequest)
      {
        case USB_STD_REQ_GET_DESCRIPTOR:
          if(setup->wValue >> 8 == HID_REPORT_DESC)
          {
            if(setup->wIndex == 0)
            {
            len = MIN(CUSTOMHID_SIZ_REPORT_DESC, setup->wLength);
            buf = (uint8_t *)CustomHID_ReportDescriptor;
            usbd_ctrl_send(pudev, (uint8_t *)buf, len);
            }
            if(setup->wIndex == 1)
            {
            len = MIN(CONSUMER_SIZ_REPORT_DESC, setup->wLength);
            buf = (uint8_t *)ConsumerHID_ReportDescriptor;
            usbd_ctrl_send(pudev, (uint8_t *)buf, len);
            }
            if(setup->wIndex == 2)
            {
            len = MIN(SYNCHID_SIZ_REPORT_DESC, setup->wLength);
            buf = (uint8_t *)SyncHID_ReportDescriptor;
            usbd_ctrl_send(pudev, (uint8_t *)buf, len);
            }
          }
          else if(setup->wValue >> 8 == HID_DESCRIPTOR_TYPE)
          {
            if(setup->wIndex == 0)
            {
             len = MIN(9, setup->wLength);
              buf = (uint8_t *)g_usbd_configuration + 18;
              usbd_ctrl_send(pudev, (uint8_t *)buf, len);
            }
            if(setup->wIndex == 1)
            {
             len = MIN(9, setup->wLength);
              buf = (uint8_t *)g_usbd_configuration + 43;
              usbd_ctrl_send(pudev, (uint8_t *)buf, len);
            }
            if(setup->wIndex == 2)
            {
             len = MIN(9, setup->wLength);
              buf = (uint8_t *)g_usbd_configuration + 68;
              usbd_ctrl_send(pudev, (uint8_t *)buf, len);
            }
          }
          else
...
static usb_sts_type class_ept0_rx_handler(void *udev)
{
  ...
  /* ...user code... */
  if( pcshid->hid_state == HID_REQ_SET_REPORT)//command
  {
    /* hid buffer process */
...
static void usb_hid_buf_process(void *udev, uint8_t *report, size_t len)
{
  Commands_HandleUsbCommand(report, bootloader_len);
}
/**
  * @}
  */
